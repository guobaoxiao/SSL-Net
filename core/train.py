...
def train_step(step, optimizer, model, match_loss, data):
    ...
    logits, ys_ds, e_hat, y_hat,indices_1,indices_2 = model(xs, ys)
    loss = 0
    loss_val = []
    #for i in range(len(y_hat)):
    loss, geo_loss, cla_loss, l2_loss, _, _ = match_loss.run(step, data, logits, ys_ds, e_hat, y_hat,indices_1,indices_2)
    #loss += loss_i
