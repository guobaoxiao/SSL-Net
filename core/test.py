...
def test_sample(args):
    ...
    _valid_th = np.sort(_valid)[::1][config.obj_top_k] #升序排序[::1],降序[::-1]
...
def test_process(mode, model, cur_global_step, data_loader, config):
    ...
    with torch.no_grad(): 
        for test_data in loader_iter:
            ...
            test_ys = test_data['ys'].squeeze(-1)
            
            #--------------gflopts
            #input1=test_data
            #macs, params = profile(model, inputs=(test_xs,test_ys))
            #print("flops:", macs)
            #print("para:", params)
            ...
            logits, ys_ds, e_hat, y_hat,indices_1,indices_2 = model(test_xs, test_ys)
            #y_hat, e_hat = res_logits[-1], res_e_hat[-1]
            loss, geo_loss, cla_loss, l2_loss, prec, rec = match_loss.run(cur_global_step, test_data, logits, ys_ds, e_hat, y_hat,indices_1,indices_2)
            ...
            for batch_idx in range(len(e_hat)):
                ...
                pool_arg += [(test_xs, test_data['Rs'][batch_idx].detach().cpu().numpy(), \
                              test_data['ts'][batch_idx].detach().cpu().numpy(), e_hat[0][batch_idx].detach().cpu().numpy(), \
